...
model1 = torch.hub.load('ultralytics/yolov5', 
                        'custom', 
                        path='/Users/paantur/Documents/Учеба/Data Science/Elbrus Bootcamp/Final project/weights_model1.pt', 
                        force_reload=True)
...
model2.fc = nn.Linear(in_features=2048, 
                      out_features=102)
model2.load_state_dict(torch.load('/Users/paantur/Documents/Учеба/Data Science/Elbrus Bootcamp/Final project/weights_model2.pt', 
                                  map_location=torch.device('cpu')))
model2.eval()

# Load the dataset with classes
model2_df = pd.read_excel('/Users/paantur/Documents/Учеба/Data Science/Elbrus Bootcamp/Final project/classes_description.xlsx')
...
async def classify(msg: types.Message):
    ...
    if len(model1_crop) == 0:
        await msg.answer('No sign has been found. Please try sending another photo.')
    else:
        try:
            ...
            await msg.answer('Error. Please try again.')
